describe("Basic user flow for Website", () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let shadowProduct = await prodItem.getProperty("shadowRoot");
    const shadowBtn = await shadowProduct.$("button");
    await shadowBtn.click();
    const text = await shadowBtn.getProperty("innerText");
  ...
  it("Checking number of items in cart on screen", async () => {
    ...
    for (let i = 1; i < prodItems.length; i++) {
      let shdwRt = await prodItems[i].getProperty("shadowRoot");
      let shdwBtn = await shdwRt.$("button");
      await shdwBtn.click();
    }
    const cartCount = await page.$("#cart-count");
    const cartText = await cartCount.getProperty("innerText");
    const value = await cartText.jsonValue();
  ...
  it("Checking number of items in cart on screen after reload", async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      let shdwRt = await prodItems[i].getProperty("shadowRoot");
      let shdwBtn = await shdwRt.$("button");
      let btnText = await shdwBtn.getProperty("innerText");
      let btnTextValue = await btnText.jsonValue();
